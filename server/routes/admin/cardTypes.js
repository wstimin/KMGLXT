...
const KINDS = ['duration', 'times', 'money', 'permanent'];

function parseType(body, hasId) {
  const { name, kind, days, times, amount, price, scope_projects, status, sort } = body || {};
  ...
  return { name: String(name).trim(), kind: k, days: d, times: t, amount: a, price: p, scope, status: st, sort: s };
}
...
router.get('/options', authAdmin, (req, res) => {
  const db = getDb();
  const rows = db.prepare('SELECT id, name, kind, days, times, amount FROM card_types WHERE status = 1 ORDER BY sort, id').all();
...
router.post('/create', authAdmin, asyncWrap(async (req, res) => {
  ...
  const db = getDb();
  const info = db
    .prepare('INSERT INTO card_types (name, kind, days, times, amount, price, scope_projects, status, sort, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)')
    .run(t.name, t.kind, t.days, t.times, t.amount, t.price, t.scope, t.status, t.sort, now());
...
router.put('/:id', authAdmin, asyncWrap(async (req, res) => {
  ...
  db.prepare('UPDATE card_types SET name=?, kind=?, days=?, times=?, amount=?, price=?, scope_projects=?, status=?, sort=? WHERE id=?')
    .run(t.name, t.kind, t.days, t.times, t.amount, t.price, t.scope, t.status, t.sort, id);
